...
chrome.storage.sync.get({ enabled: true, paused: false, hero: 'catgirl', dlgLang: 'ru', devLedges: false, yasiaDevMode: false }, (s) => { cb.checked = s.enabled; const pcb = document.getElementById('paused'); if (pcb) pcb.checked = !!s.paused; let h = (s.hero === 'noema' || !HEROES.some((x) => x.id === s.hero)) ? 'catgirl' : s.hero; if (h !== s.hero) chrome.storage.sync.set({ hero: h }); markHero(h); lang = (s && s.dlgLang === 'en') ? 'en' : 'ru'; const dv = document.getElementById('devLedges'); if (dv) dv.checked = !!s.devLedges; applyDevMode(!!s.yasiaDevMode); localize(); });
...
// режим разработчика: вкл = видны дев-секция попапа и скрытые навыки в окне Яси; выкл = продакшен-вид (страница подхватит через onChanged)
function applyDevMode(on) {
  const t = document.getElementById('devMode'); if (t) t.checked = !!on;
  const sec = document.getElementById('dev-sec'); if (sec) sec.hidden = !on;
}
{ const dm = document.getElementById('devMode'); if (dm) dm.addEventListener('change', () => { applyDevMode(dm.checked); chrome.storage.sync.set({ yasiaDevMode: dm.checked }); }); }
{ const rb = document.getElementById('dev-reset'); if (rb) rb.addEventListener('click', () => { xp = 0; chrome.storage.local.set({ xp: 0, xpDay: null }); render(); }); }   // сброс прогресса: страница подхватит xp через onChanged
...
function localize() {
  ...
  set('t-feed', t.feed); set('t-hint', t.hint); set('t-dev', t.dev); set('t-devmode', t.devModeLb); set('dev-reset', t.devReset); set('t-devreset-hint', t.devResetHint); set('t-devledges', t.devLedges); set('t-devledges-hint', t.devLedgesHint); set('t-ai', '🤖 ' + t.ai); set('ai-toggle', t.connect);
