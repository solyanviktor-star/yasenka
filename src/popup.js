...
const HEROES = (Array.isArray(CFG.HEROES) && CFG.HEROES.length) ? CFG.HEROES : [{ id: 'catgirl', name: 'Yasia' }, { id: 'noema', name: 'Noema' }];
...
chrome.storage.sync.get({ enabled: true, hero: 'catgirl', dlgLang: 'ru' }, (s) => { cb.checked = s.enabled; markHero(s.hero); lang = (s && s.dlgLang === 'en') ? 'en' : 'ru'; localize(); });
...
function localize() {
  ...
  set('t-feed', t.feed); set('t-hint', t.hint); set('t-ai', t.ai); set('ai-toggle', t.connect);
