...
(() => {
  ...
  Yasia.config = Object.freeze({
    ...
    FEED_XP: 12,
    LEVEL_XP: [0, 40, 120, 280, 550, 950, 1500, 2200, 3100, 4200], // пороги опыта для уровней 1..10 (длинная кривая: уровень = недели заботы, не клики)
    MAX_LEVEL: 10,
    HUNGER_HUNGRY: 55,    // выше -> просит еду
    HUNGER_STARVING: 82,  // выше -> грустит/вредничает
    // экономика опыта: уровень должен мерить ЗАБОТУ ВО ВРЕМЕНИ, а не количество кликов
    XP: {
      dayCap: 90,          // мягкий дневной потолок XP: быстрее чем ~за неделю до 5 ур. не накрутить
      needyMul: 2,         // забота «когда ей плохо» (кормить голодную) ценится вдвое
      petPerHour: 3,       // XP только за первые N поглаживаний в час (дальше — ласка без опыта)
      feedMinHunger: 22,   // ниже этого голода сытая ОТКАЗЫВАЕТСЯ есть (еда не тратится, XP нет)
      replyXp: 10,         // бонус за первый черновик реплая за день (осмысленное применение Яси)
    },
    ...
    STREAK: { xpPerDay: 4, xpCap: 40 },   // усилен: регулярность — главный источник XP в длинной кривой
